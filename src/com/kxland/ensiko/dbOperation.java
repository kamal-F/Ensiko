...
public class dbOperation extends SQLiteAssetHelper {
  ...
  //getAllSayur
    public List<Sayur> getAllSayur() {
        List<Sayur> sayurList = new ArrayList<Sayur>();
        // Select All Query
        String selectQuery = "SELECT  * FROM " + "ensiko" + " WHERE jenis='sayur'";
        ...
        if (cursor.moveToFirst()) {
            do {
            	...
            	sayur.setIDx(Integer.parseInt(cursor.getString(0)));
            	sayur.setKategori(cursor.getString(1));
            	sayur.setSayur(cursor.getString(2));            	
            	sayur.setGambar(BitmapFactory.decodeByteArray( cursor.getBlob(3), 0, cursor.getBlob(3).length));
            	sayur.setdeskripsi(cursor.getString(4));
                // Adding buah to list
            	sayurList.add(sayur);
            } while (cursor.moveToNext());
        }
